...
from ddsp.sphere.net.interp import sphere_interpolate, get_bi_interp

from ddsp.sphere.net.utils import get_neighs_order, get_upconv_index
...
class SUNet(nn.Module):
    ...
    def __init__(
        ...
        self.deconv5 = conv_block(
            C_hid[4] + C_hid[4], C_hid[4], neigh_orders[4]
        )
        self.deconv4 = conv_block(
            C_hid[4] + C_hid[3], C_hid[3], neigh_orders[3]
        )
        self.deconv3 = conv_block(
            C_hid[3] + C_hid[2], C_hid[2], neigh_orders[2]
        )
        self.deconv2 = conv_block(
            C_hid[2] + C_hid[1], C_hid[1], neigh_orders[1]
        )
        self.deconv1 = conv_block(
            C_hid[1] + C_hid[0], C_hid[0], neigh_orders[0]
        )
...
class SphereDeform(nn.Module):
    ...
    def forward(self, feat_in, vert_in, n_steps=7):
        ...
        phi = (
            phi[:, self.neigh_orders[0]]
            .reshape(-1, phi.shape[1], 7, 3)
            .mean(-2)
        )
