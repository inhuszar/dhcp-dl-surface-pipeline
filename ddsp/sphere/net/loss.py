...
def edge_distortion(vert_sphere, vert_surf, edge):
    ...
    """
    # compute edge length
    edge_len_sphere = (vert_sphere[:, edge[0]] - vert_sphere[:, edge[1]]).norm(
        dim=-1
    )
...
def geodesic_distortion(vert_pred, vert_gt, n_samples=500000, radius=1.0):
    """
    ...
    d_pred = (
        radius
        * torch.acos(torch.clamp(1.0 - d_pred / 2, min=-1 + 1e-6, max=1 - 1e-6))
        + 1e-8
    )
    d_gt = (
        radius
        * torch.acos(torch.clamp(1.0 - d_gt / 2, min=-1 + 1e-6, max=1 - 1e-6))
        + 1e-8
    )
