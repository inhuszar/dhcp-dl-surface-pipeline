...
class LinearInterpolator(nn.Module):
    ...
    def __init__(self, inshape=[112, 224, 160], device="cpu"):
        ...
        self.linear_grid = torch.stack(linear_grid).unsqueeze(0).to(device)
        self.min_bound = torch.Tensor([1e-5, 1e-5, 1e-5]).to(device)
        self.max_bound = torch.Tensor(inshape).to(device) - 1 - 1e-5
    ...
    def forward(self, vert, vol):
        ...
        # 3D interpolation
        value = (1 - u[:, :, 0])[..., None, None] * value[:, :, 0] + u[:, :, 0][
            ..., None, None
        ] * value[:, :, 1]
        value = (1 - u[:, :, 1])[..., None] * value[:, :, 0] + u[:, :, 1][
            ..., None
        ] * value[:, :, 1]
        value = (1 - u[:, :, 2]) * value[:, :, 0] + u[:, :, 2] * value[:, :, 1]
...
class CubicInterpolator(nn.Module):
    ...
    def __init__(self, inshape=[112, 224, 160], device="cpu"):
        ...
        self.cubic_grid = torch.stack(cubic_grid).unsqueeze(0).to(device)
        self.min_bound = torch.Tensor([1.0 + 1e-5, 1.0 + 1e-5, 1.0 + 1e-5]).to(
            device
        )
        self.max_bound = torch.Tensor(inshape).to(device) - 2 - 1e-5
    ...
    def forward(self, vert, vol):
        ...
        u = (v - vid)[..., None]
        # distance to voxels
        u = torch.cat([torch.ones_like(u), u, u**2, u**3], dim=-1)
