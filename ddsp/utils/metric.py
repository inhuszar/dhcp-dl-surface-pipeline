...
from ddsp.utils.mesh import laplacian_smooth, vert_normal
...
def cortical_thickness(vert_white, vert_pial):
    """
    Estimate cortical thickness between wm and pial surfaces.
...
def curvature(vert, face, curv_type="mean", smooth_iters=0):
    ...
    workbench commandline. For the original code, please see:
    ...
    basis = (
        (normal[:, :, 0].abs() > normal[:, :, 1].abs()).unsqueeze(-1).float()
    )
...
def myelin_map(subj_dir, hemi="left", myelin_sigma=MYELIN_SIGMA):
    """
    Estimate the myelin map from the T1/T2 ratio, cortical ribbon,
    cortical thickness and mid-thickness surface.
    ...
    subprocess.run(
        [
            ...
            os.path.join(subj_dir, "T1wDividedByT2w.nii.gz"),
            os.path.join(subj_dir, f"hemi-{hemi}_midthickness.surf.gii"),
            os.path.join(subj_dir, f"hemi-{hemi}_myelinmap.shape.gii"),
            "-myelin-style",
            os.path.join(subj_dir, "ribbon.nii.gz"),
            os.path.join(subj_dir, f"hemi-{hemi}_thickness.shape.gii"),
            str(myelin_sigma),
        ],
        shell=True,
    )
    ...
    myelin_gii = os.path.join(subj_dir, f"hemi-{hemi}_myelinmap.shape.gii")
    myelin = nib.load(myelin_gii).agg_data()
    os.remove(myelin_gii)
    return myelin


def smooth_myelin_map(subj_dir, hemi="left", smooth_sigma=SMOOTH_SIGMA):
    """
    ...
    subprocess.run(
        [
            ...
            os.path.join(subj_dir, f"hemi-{hemi}_myelinmap.shape.gii"),
            str(smooth_sigma),
            os.path.join(subj_dir, f"hemi-{hemi}_smoothed_myelinmap.shape.gii"),
        ],
        shell=True,
    )

    # Remove the GIfTI file, and save it later to change colormap
    smoothed_myelin_gii = os.path.join(
        subj_dir, f"hemi-{hemi}_smoothed_myelinmap.shape.gii"
    )
    smoothed_myelin = nib.load(smoothed_myelin_gii).agg_data()
    os.remove(smoothed_myelin_gii)
