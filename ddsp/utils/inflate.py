...
def wb_generate_inflated_surfaces(
    ...
    subprocess.run([
        ...
        os.path.join(save_dir, f"hemi-{hemi}_midthickness.surf.gii"),
        os.path.join(save_dir, f"hemi-{hemi}_inflated.surf.gii"),
        os.path.join(save_dir, f"hemi-{hemi}_vinflated.surf.gii"),
        "-iterations-scale",
        str(iter_scale),
    ], shell=True)
...
def mris_inflate(
    ...
    dist_0 = (vert[0, edge_n[1]] - vert[0, edge_n[0]]).norm(
        dim=-1, keepdim=True
    )
    ...
    n_avgs = [
        2**i for i in range(max_level - 2, np.maximum(min_level, 1) - 2, -1)
    ]
    if min_level == 0:
        n_avgs.append(0)
    disp = 0
    ...
    for level in range(len(n_avgs)):
        for t in range(n_steps):
            ...
            e_t = (
                vert_t[0, edge_n[1]] - vert_t[0, edge_n[0]]
            )  # direction vector

            # distortion=(dist_t-dist_0)^2, build sparse matrix
            distort = torch.sparse_coo_tensor(
                edge_n,
                (dist_t - dist_0 / scale_t) / dist_t * e_t,
                (n_vert, n_vert, 3),
            ).unsqueeze(0)
            grad_distort = (
                torch.sparse.sum(distort, dim=-2).to_dense() / degree_n
            )

            # take out the components of normal
            g_dot_n = (grad_distort * normal_t).sum(
                -1, keepdim=True
            )  # dot product
            ...
            for i in range(n_avgs[level]):
                grad_distort = (grad_distort + A.bmm(grad_distort)) / (
                    degree + 1
                )
            ...
            if verbose:
                ...
                J_inflate = (
                    torch.sparse.sum(Dist2, dim=-1).to_dense().unsqueeze(-1)
                )
                ...
                J_distort = (
                    torch.sparse.sum(Delta2, dim=-1).to_dense().unsqueeze(-1)
                )
    ...
    vert_center = (
        vert_t.max(1, keepdim=True)[0] + vert_t.min(1, keepdim=True)[0]
    ) / 2
