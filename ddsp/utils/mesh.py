...
def adjacent_faces(face):
    ...
    """
    edge = torch.cat(
        [face[0, :, [0, 1]], face[0, :, [1, 2]], face[0, :, [2, 0]]], axis=0
    )  # (2|E|, 2)
    nf = face.shape[1]
...
def vert_normal(vert, face):
    ...
    f_normal_0 = torch.cross(
        v_f[:, :, 1] - v_f[:, :, 0], v_f[:, :, 2] - v_f[:, :, 0], dim=2
    )
    f_normal_1 = torch.cross(
        v_f[:, :, 2] - v_f[:, :, 1], v_f[:, :, 0] - v_f[:, :, 1], dim=2
    )
    f_normal_2 = torch.cross(
        v_f[:, :, 0] - v_f[:, :, 2], v_f[:, :, 1] - v_f[:, :, 2], dim=2
    )
...
def adjacency_matrix(face):
    ...
    A = torch.sparse_coo_tensor(
        edge, torch.ones_like(edge[0]).float(), (nv, nv)
    ).unsqueeze(0)
...
def neighbor_matrix(face, n_neighbors=2):
    ...
    A_n = torch.sparse_coo_tensor(
        edge_n, torch.ones_like(edge_n[0]).float(), (nv, nv)
    ).unsqueeze(0)
    return A_n
...
def laplacian(face):
    ...
    weight = 1.0 / degree[edge[0]]
    # normalized adjacency matrix
    A_hat = torch.sparse_coo_tensor(edge, weight, (nv, nv)).unsqueeze(0)
    ...
    D_hat = torch.sparse_coo_tensor(
        self_edge, torch.ones_like(self_edge[0]).float(), (nv, nv)
    ).unsqueeze(0)
    L = D_hat - A_hat
    return L
...
def cot_laplacian(vert, face, eps=1e-12):
    ...
    A = torch.sparse.FloatTensor(idx, cot.reshape(-1), (nv, nv))
    ...
    self_edge = torch.arange(nv)[None].repeat([2, 1]).to(face.device)
    D = torch.sparse_coo_tensor(self_edge, degree, (nv, nv))
    L = (D - A).unsqueeze(0)
...
def laplacian_smooth(vert, face, lambd=1.0, n_iters=1):
    """
...
def cot_laplacian_smooth(vert, face, lambd=1.0, n_iters=1):
    """
...
def area_weighted_smooth(vert, face, strength=1.0, n_iters=1):
    """
    ...
    v_f_id = torch.cat(
        [
            torch.stack([face[0, :, 0], fid]),
            torch.stack([face[0, :, 1], fid]),
            torch.stack([face[0, :, 2], fid]),
        ],
        dim=-1,
    )
    ...
    A = torch.sparse_coo_tensor(
        v_f_id, torch.ones_like(v_f_id[0]).float(), (nv, nf)
    ).unsqueeze(0)
