...
def save_gifti_surface(
    vert, face, gifti_path, hemi="left", surf_type="wm"
):
    """
    ...
    Inputs:
        ...
        - gifti_path: path to the output GIfTI file, string
        - hemi: ['left', 'right']
    ...
    # meta data
    vert_meta_dict = {
        "<![CDATA[AnatomicalStructurePrimary]]>": "<![CDATA["
        + _hemi
        + "]]>",
        "<![CDATA[AnatomicalStructureSecondary]]>": "<![CDATA["
        + _surf_type
        + "]]>",
    ...
    with open(gifti_path, "wb") as f:
        f.write(gii_file.encode("utf-8"))
    # nib.save(gii_surf, gifti_path)


def save_gifti_metric(metric, gifti_path, hemi="left", metric_type="curv"):
    """
    Save gifti metric (.shape.gii).

    Inputs:
        - metric: mesh metric, (|V|) numpy.float32
        - gifti_path: output file, string
        - hemi: ['left', 'right']
    ...
    gii_metric_arr = gifti.GiftiDataArray(
        metric, intent="normal", meta=metric_meta
    )
    ...
    with open(gifti_path, "wb") as f:
        f.write(gii_file.encode("utf-8"))
    # nib.save(gii_metric, gifti_path)


def create_wb_spec(subj_dir):
    """
    Create workbench specification file (.spec) for visualization
    ...
    for hemi in ["left", "right"]:
        for file_type in ["surf", "shape"]:
            ...
            file_list = glob.glob(
                os.path.join(subj_dir, f"hemi-{hemi}*.{file_type}.gii")
            )
            # add to xml tree
    ...
    file_name = os.path.join(subj_dir, "T2w_restore_brain.nii.gz")
    # add to xml tree
    et.SubElement(
        caret,
        "DataFile",
